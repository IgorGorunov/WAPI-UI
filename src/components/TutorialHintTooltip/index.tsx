...
const TutorialHintTooltip: React.FC<TutorialHintTooltipPropsType> = ({ hint, classNames = '', children, position = 'center', noMargin = false, forBtn = false }) => {
    return (
        <div className={`${styles['tutorial-hint-tooltip']} tutorial-hint-tooltip ${classNames} ${!!hint ? `${styles['has-hint'] || 'has-hint'}` : `${styles['no-hint'] || 'no-hint'}`} ${noMargin ? `${styles['no-margin'] || 'no-margin'}` : ''}`}>
            {children}
