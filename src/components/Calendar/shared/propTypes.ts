...
const isRange = PropTypes.arrayOf(
  PropTypes.oneOfType([PropTypes.instanceOf(Date), PropTypes.oneOf([null])])
    .isRequired
...
export const isView: Requireable<View> = function isView(
  props,
  propName,
  componentName
) {
  ...
  if (
    view !== undefined &&
    (typeof view !== "string" || allViews.indexOf(view) === -1)
  ) {
    return new Error(
